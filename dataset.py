...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def _prepare_conversations(self):
        ...
        tokenized_dialogs = []
        for i, (log, label) in enumerate(tqdm(self.dataset_walker, disable=self.args.local_rank not in [-1,
                                                                                                        0])):  # only show progress bar in one process
            dialog = {}
            dialog["id"] = i
            dialog["log"] = log
